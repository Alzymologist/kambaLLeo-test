...
// Список всех наших приложений, которые бывают
pub enum Screen {
    PinEntry,
...
impl <P: Platform> UIState<P> {
    pub fn new(mut platform: P) -> Self {
        ...
        if platform.entropy_display().0.is_empty() {
            UIState {
		// Вот в этом месте мы указываем, какое приложение (Screen) вызвать по умолчанию при старте
                // screen: Screen::OnboardingRestoreOrGenerate,
                screen: Screen::LLeo, // OnboardingRestoreOrGenerate, LLeo
                platform: platform,
            }
        } else {
            UIState {
