...
use crate::{prn,prnln,LOOP_COUNTER}; // lleo
...
use efm32pg23_fix::{NVIC,Interrupt, emu_ns::decbod::DECBOD_SPEC};
...
impl BufferStatus {
    pub fn new() -> Self {
        ...
        prnln(format!("\nwwww1: BufferStatus.new() -> RhW0")); //lleo
    ...
    pub fn pass_if_done7(&mut self) -> Result<(), BufferError> {
        ...
        // ЕСЛИ ПОДНЕСЛИ ТЕЛЕФОН!!!!!!!!!!!!!
        // ИЛИ ЕСЛИ ЧТО-ТО ПРОЧИТАЛИ!!!!!!!!!!!!!
        prnln(format!("wwww2: BufferStatus.pass_if_done7({:#?}) -> {:#?}",self.clone(),new_self.clone())); //lleo
    ...
    pub fn is_write_halted(&self) -> bool {
        ...
        prnln(format!("wwww5: BufferStatus.is_write_halted({:#?}) -> {:#?}",self.clone(),lleo)); //lleo
        // ################################
...
impl <'a> NfcReceiver<'a> {
    ...
    pub fn new(nfc_buffer: &'a [u16; 3*BUF_THIRD], public_memory: Option<[u8; 32]>) -> Self {
        ...
        prnln(format!(" wwww11 ")); //lleo
        // prnln(format!(" len: {} ",nfc_buffer.len())); //lleo
...
// const mut debcounter:u32 = 0; //lleo


    pub fn advance(&mut self, voltage: i32) -> Option<NfcResult> {
//        if (voltage < NFC_MIN_VOLTAGE) { return None; } // DBLLEO

if voltage > 9000 {
    unsafe {
        if LOOP_COUNTER % 10000 == 0 { prn(format!("#")); }
    }
} //lleo
